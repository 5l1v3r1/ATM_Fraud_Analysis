...
reaches in and examines the untold riches stored inside.
...
Freon works on these locks. Some outlaws empty a can of freon(halocarbon) into 
an ATM lock, pound a screwdriver into the key way, and wrench the lock out, 
motor-driven ACE lock pick will vibrate pins into the right positions within a few minutes.

Method 4: ACETYLENE & DRILLS
============================

ATMs are notorisly vulnerable to attacks using acetylene torches(google) with most
ATMs no more than 5 minutes are required for the entire job.
...
"Tapping" or "wiretapping" consists of the unauthorized electronic monitoring
...
(located in the banks data processing center) can be tapped anywhere 
between the two.
...
or antenna is used to pick up the EMI generated(Electromagnetic interference, 
radio-frequency interference). EMI frequently affects AM radios. by the signal, 
and there is no physical connection between the commo circuit and the line. 
...
"active tap" is one in which the tap also interferes (changes,adds to or deletes)
the tapped signal in some way. Active taps are more sophisted. 
A typical ATM active tap is one that records a signal, the later plays
...
to a credit card. The other end will proxy a reader's requests over WiFi/Net to the relay 
end.
...
remote party picked up the phone as automated operator.
